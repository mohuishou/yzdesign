...
class App_YunQianLevelDao extends PwBaseDao {
	
    protected $_table = 'app_yunqian_level';
    protected $_pk = 'dj';
	protected $_dataStruct = array('dj', 'hits', 'title');
  	...
  	public function countByWhere($where){
		$sql = $this->_bindSql('SELECT COUNT(*) AS count FROM %s WHERE dj %s ', $this->getTable(), $where);
		$smt = $this->getConnection()->createStatement($sql);
		return $smt->getValue(array());
	}
	
	public function getList($where, $limit, $offset){
		$sql = $this->_bindSql('SELECT * FROM %s WHERE dj %s ORDER BY dj asc  %s', $this->getTable(), $where, $this->sqlLimit($limit, $offset));
		$smt = $this->getConnection()->createStatement($sql);
